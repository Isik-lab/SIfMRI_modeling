...
class VisionOOORSA:
    ...
    def load_data(self):
        return Benchmark(stimulus_data=f'{self.data_dir}/interim/ReorganizefMRI/stimulus_data.csv')
    ...
    def run(self):
        try:
            if os.path.exists(self.out_file) and not self.overwrite:
                ...
                # Load data and sort
                annotations = pd.read_csv(f"{self.data_dir}/raw/utils/annotations.csv")
                train_list = pd.read_csv(f"{self.data_dir}/raw/utils/train.csv")
                annotations = annotations.reset_index().rename(columns={"index": "vid_id"})
                train_behavior = annotations[annotations['video_name'].isin(train_list['video_name'])]
                train_idx = train_behavior['vid_id'].to_list()
